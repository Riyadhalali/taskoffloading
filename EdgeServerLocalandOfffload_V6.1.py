...
class CloudEnvironment:
    def __init__(self, env, num_servers, cpu_power, memory, edge_servers):
        self.env = env
        ...
        self.rl_agent = RLAgent(state_size=10000, action_size=len(edge_servers) + 1)  # +1 for cloud processing

    def receive_task(self, task):
        self.env.process(self.decide_and_process(task))

    def decide_and_process(self, task):
        state = self.get_state(task)
        action = self.rl_agent.get_action(state)

        if action == len(self.edge_servers):  # Process on cloud
            yield self.env.process(self.process_on_cloud(task))
        else:  # Send to edge server
            edge_server = self.edge_servers[action]
            yield self.env.process(self.send_to_edge(task, edge_server))

    def get_state(self, task):
        edge_loads = [min(int(server.current_load / server.max_load * 10), 9) for server in self.edge_servers]
        ...
        return sum(edge_loads) * 1000 + network_state * 100 + complexity_state * 10 + priority_state
    ...
    def process_on_cloud(self, task):
        with self.servers.request() as request:
            ...
            self.processed_tasks.append(
                ('Cloud', start_time, end_time, task.duration, task.complexity, task.priority, 'Cloud'))
            print(f'Cloud processed task (priority {task.priority}, complexity {task.complexity}) at {self.env.now}')
...
# Create edge servers
num_edge_servers = 3
edge_servers = [EdgeServer(env, f'EdgeServer_{i}', None, cpu_power=2.0, memory=8, max_concurrent_tasks=5) for i in
                range(num_edge_servers)]

# Create cloud environment with specified capabilities
cloud_env = CloudEnvironment(env, num_servers=2, cpu_power=32.0, memory=64, edge_servers=edge_servers)

# Update edge servers with cloud environment
for server in edge_servers:
    server.cloud_env = cloud_env
...
# Send all tasks to the cloud
for task in tasks:
    cloud_env.receive_task(task)
...
for _, row in df.iterrows():
    print(
        f"Device: {row['Device']}, Type: {row['Type']}, Priority: {row['Priority']}, Complexity: {row['Complexity']}, Processing Time: {row['Processing Time']}")
...
# Print the Q-table for the cloud
print("\nQ-table for Cloud:")
...
print(f"\nTotal reward for Cloud RL agent: {cloud_env.rl_agent.total_reward}")